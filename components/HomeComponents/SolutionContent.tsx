  ...
  return (
    <div className={`${bgColor} px-2.5 py-4 xl:py-10`}>
      <div className="w-full h-full relative">
        ...
        <div
          className={`absolute left-0 bottom-0 font-bold text-lg flex border-x-0 border-t-0 border-b border-solid border-black cursor-pointer`}
          onClick={() => router.push(moreUrl)}
        >
          <span className="hover:text-red-500">Tìm hiểu thêm</span>
          <DoubleRightOutlined className="ml-6 text-red-500 text-base" />
        </div>
      </div>
    </div>
  );
